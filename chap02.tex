...
não acrescentar diferenças significativas esperadas no estudo do presente
trabalho, é utilizado o método de \textit{boustrophedon} para planejamento do
...
excitação entre $10~dB$ a $20~dB$. A Figura~\ref{fig::range_hammer} apresenta um
resultado típico da resposta em frequência da excitação por impacto, indicando a
...
Logo, para estruturas grandes, normalmente são necessários martelos de massa
elevada, a fim de fornecer energia suficiente para excitar uma faixa
...
flexíveis conectados por juntas para formar o
mecanismo. Mas na maioria dos casos, a flexibilidade estrutural não é a intenção
de projeto, mas consequência da construção do manipulador a partir de requisitos
sobre a massa e geometria de seus componentes~\cite{moallem2000flexible}.

Por causa de requisitos de desempenho, como alta velocidade e
acurácia, e aplicações espaciais envolvendo grandes estruturas, mas utilizando
materiais leves para economia de combustível, a consideração da flexibilidade
estrutural em robótica torna-se relevante. A partir da década de 1980, diversos
textos foram publicados~\cite{sunada1983dynamic}, \cite{bayo1987finite} e
...
restringiu-se ao braço robótico flexível de apenas um ou dois elos.

Técnicas de controle destes manipuladores foram desenvolvidas a partir de então,
a fim de evitar evitar falhas de posicionamento do efetuador e de perseguição da
trajetória.
...
\subsubsection{Manipuladores sobre bases flexíveis}

% Macro-micro manipuladores
