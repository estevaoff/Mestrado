...
\begin{align}
	...
	\label{eq::posjnutas} \\
	^{Z}\mathbf{pcm}^{k} &= ~^{Z}\mathbf{p}^{k-1} + ~^{k}\mathbf{pcm}
	\label{eq::pcm}
\end{align}
%
...
posição do centro de massa do corpo $k$, tal que $k$ varia em \{S,L,U,R,B\},
...
robô, $q1 = \ldots = q6 = 0$; e a Figura~\ref{fig::amb3D_posB} para a
...
%
$$
S_{L}^{Z} = \left( \begin {array}{ccc} 0&-{\it \dot{q}2}&-\sin \left( {\it q2}
 \right) {\it \dot{q}1}\\ \noalign{\medskip}{\it \dot{q}2}&0&-\cos \left( {\it 
q2} \right) {\it \dot{q}1}\\ \noalign{\medskip}\sin \left( {\it q2}
 \right) {\it \dot{q}1}&\cos \left( {\it q2} \right) {\it \dot{q}1}&0
\end {array} \right)
$$
%
...
%
$$
^{Z}\boldsymbol{\omega}^{L} = [-{\it \dot{q}1},\sin \left( {\it q1} \right) {\it \dot{q}2},-\cos
\left( {\it q1} \right) {\it \dot{q}2}]
$$
%
...
\subsubsection{Parâmetros de controle PID com compensação da gravidade}
...
Para utilização deste método em manipuladores robóticos é necessário modelar os
torques de compensação da gravidade em cada corpo, que varia de acordo com a
configuração das juntas. Estes serão denominados torques de freio, já que para
qualquer configuração estes torques anulam o efeito do peso dos elos, atuando
nas juntas do robô.
A equação geral para os torques de freio pode ser escrita como:
%
\begin{equation}
	\mathbf{Tb}_k = \mathbf{pcm}_k \times (m_k \cdot \mathbf{g}) + \mathbf{p}^k
	\times (\sum_{r=k+1}^{B} m_r \cdot \mathbf{g})
\end{equation}
%

Logo, define-se as equações dos torques de controle para cada junta, em função
da diferença entre os parâmetros instantâneos e os de referência.
O subíndice $k$ representa a lista de juntas, localizada na origem de cada SC,
variando em \{S,L,U,R,B\} e o subíndice $i$ o número da coordenada generalizada
associada.
%
\begin{equation}
...
\begin{figure}[h]
	\centering 
 	\includegraphics[width=0.85\textwidth]{figs/pid_pi_sobre_2}
...
\medskip \noindent {\tt > for k from 1 to 5 do \\
		 \indent PID\_||{corpo[k]}:= Kp*(q||k - q||k||ref) + \\
		 \indent + Ti*int(q||k - q||k||ref, t) + Td*(u||k - u||k||ref) \\
		 end do:}
...
\begin{align}
	...
	\mathbf{Mex_{S}} &= \mathbf{TjZS} - \mathbf{TjSL} - \mathbf{TbS}	\\
	\mathbf{Mex_{L}} &= \mathbf{TjLU} - \mathbf{TjSL} - \mathbf{TbL}	\\
	\mathbf{Mex_{U}} &= \mathbf{TjUR} - \mathbf{TjLU} - \mathbf{TbU}	\\
	\mathbf{Mex_{R}} &= \mathbf{TjRB} - \mathbf{TjUR} - \mathbf{TbR}	\\
	\mathbf{Mex_{B}} &= - \mathbf{TjRB} - \mathbf{TbB}					\label{eq::mexf} 
\end{align}
%
...
de posição e de orientação, de acordo com a equação~\ref{eq::pose}.
%
\begin{equation}
...
\begin{figure}[h]
	\centering 
 	\includegraphics[width=0.60\textwidth]{figs/elbowupdown}
...
As trajetórias para representar este processo serão simplificadas, pela
consideração de que as superfícies são planas e que pode-se representar a
trajetória totalmente naquele plano.

A trajetória será modelada por um conjunto de funções de referência no tempo,
para cada coordenada generalizada. Portanto, são utilizadas as soluções da
cinemática inversa, desenvolvidadas na seção anterior. 
...
\begin{figure}[h!]
	\centering 
 	\includegraphics[width=0.45\textwidth]{figs/trajec3D_600x500x10}
...
que na descontiuidade a função é indefinida.
...
($t<1~s$), porque neste trecho o manipulador não estaria realizando o
revestimento.
...
maior poder computacional. E o resultado teria muitas informações irrelevantes
...
modelo e são aplicados no ponto virtual da base, no segundo caso suprime-se a
...
Para se chegar às equações de movimento, primeiramente, são definidos os
sistemas de coordenadas e as coordenadas generalizadas do sistema.
...
gravidade é descrito no referencial inercial SC-R.
%
\begin{equation}
	\mathbf{g} = [-9,81,~0,~0]^{R}
\end{equation}
...
uma linha ou uma coluna completa obtém-se informação suficiente para se
...
Demonstrou-se nas seções~\ref{sec::robo} e \ref{sec::base} como foram
construídos os modelos dinâmicos do robô e da base em separado. Esta seção
consiste em unir os dois sistemas MBS em um modelo acoplado.
...
apresentados os requisitos para o processo de revestimento por aspersão
...
Análise por Elementos Finitos, que forneceu sua matriz de rigidez. Valores em
$kN/m$ e $kNm/rad$.
%
\begin{equation*}
	K =
...
modelo é apresentado na Figura~\ref{fig::prp_fea}, onde os principais
...
\begin{figure}[h]
	\centering 
 	...
 	\caption{Propriedades de seção transversal dos pés da estrutura}
...
As restrições são do tipo engaste e aplicadas às extremidades dos pés de apoio
da estrutura. Os deslocamentos prescritos são então aplicados em 6 casos,
