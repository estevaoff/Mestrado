...
% Conforme demonstrado na seção~\ref{sec::ikin}, este tipo de braço robótico
...
\todo[inline]{Incluir direção do erro de orientação}
...
da equivalência do procedimento. No Anexo~\todo{Incluir código anexo} é
...
em $20~kHz$\todo{Revisar valores}, longe o suficiente da frequência de
...
das FRF's importadas. A interface de importação do programa ajuda a
identificar os dados e relacionar às direções de medição e excitação correspondentes. 

A opção \textit{Modal Analysis} no ME'Scope é utilizada para identificar as
frequências de ressonância do sistema e estimar os parâmetros modaispela função
de ajuste de curva (\textit{curve fitting}). A primeira etapa é identificar a
faixa de frequência que se deseja analisar. Em seguida, apontar os modos
ou picos naquela faixa. O programa ainda permite a contagem automática, se
fornecido um valor mínimo de mangnitude para caracterizar o modo.
...
interativa, visualizar uma animação dos modos de vibração.
