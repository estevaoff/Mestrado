...
Resumindo o que foi dito, a Figura~\ref{fig::visgeral} ilustra uma visão geral
...
\end{equation}
%
Infere-se que, para ter uma energia mais alta, ou aumenta-se a
velocidade do impacto, ou a massa do martelo. Como os modelos de martelo com
massa baixa ($<1~kg$) possuem faixa de medição também pequena, optou-se por um
martelo de massa elevada. O modelo PCB-086D50 foi escolhido para o ensaio. Este
martelo possui massa de $5,5~kg$ e faixa de medição de $\pm 25~kN$.
...
\begin{figure}[h]
	\centering 
 	...
 	\caption{Espectro de frequência do impacto do martelo PCB-086D50. Adaptado de: }
 	\label{fig::carta_badwidth}
\end{figure}
\todo[inline]{Incluir fonte da carta do martelo}
...
a Figura~\ref{fig::diagrama_bancada} um diagrama esquemático, representando os
...
Note-se que para calcular os termos devido a rotação da base, então seria
necessário excitar um momento em cada direção angular ($r_x,r_y,r_z$) e obter
$h_{i4}$ a $h_{i6}$. Entretanto, é impossível excitar, com o martelo, um momento
puro na base. Logo, estes termos não podem ser calculados diretamente. Em
compensação, o ensaio conta com aquisição simultânea de 6 sinais de saída
...
foram obtidos experimentalmente, e estão na
Tabela~\ref{tab::resultado_modal}\todo{Referenciar tabela de resultados modais}.

Selecionando-se os 10 primeiros modos, tem-se o seguinte gráfico, na
...
cumprir. Para fazer este julgamento, pode-se simplesmente testar, no modelos MBS
acoplado, uma lista de coeficientes para a matriz proporcional de amortecimento,
e verificar os resultados. Se para um amortecimento baixo, ou alto, ainda
